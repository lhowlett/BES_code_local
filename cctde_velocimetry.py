...
def calc_cctde_velocimetry(
        shot, apdpos, bes_time, fluct_data, equilib_time, equilib_R, 
        equilib_Z, equilib_psi, timeslice, xcorr_length, 
        bandpass_filter='automatic', fit_range=0.2, calc_poloidal=True, 
        calc_radial=True, remove_poloidal_average=False, use_gaussian_fit=True, 
    ...
    if bandpass_filter == 'automatic':
        for j in range(nchan):
            bpf_filt = butter_bandpass_filter(fluct_data[j], 
        ...
        for j in range(nchan):
            bpf_filt_data.append(fluct_data[j])
        ...
        try:
            ...
            for j in range(nchan):
                bpf_filt = butter_bandpass_filter(
                    fluct_data[j], (0.5 * f_samp) / xcorr_length, 
                    250.0e3, f_samp)
                bpf_filt_data.append(bpf_filt)
            print("Applied automatic bandpass filter.")
    ...
    eq_idx1 = (np.abs(equilib_time - timeslice[0])).argmin()
    eq_idx2 = (np.abs(equilib_time - timeslice[1])).argmin()
    # only calculate equilibria for given timeslice
    cut_eq_times = equilib_time[eq_idx1-1:eq_idx2+2]
        ...
        for eq_idx in range(len(cut_eq_times)):
            ...
            for row in range(nrows):
                for col in range(ncols):
                    ...
                    # at that timepoint
                    eq_array[row, col] = map_channel_to_flux_surfaces(
                        equilib_time, equilib_R, equilib_Z, equilib_psi, 
                        ...
                        for i in cols_to_calc:
                            for k in rows_to_calc:
                                ...
                                for ccc in range(ncols):
                                    if eq_radial_chans_to_use[refch, ccc] > -1:
                                        chans_1.append(
